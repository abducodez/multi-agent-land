...
![The same cast with "Read their minds" toggled on: each card now shows the public line plus the private thought beneath it — the pocket-actor saying "Please do not applaud yet; my shadow is still rehearsing" while privately scheming.](images/mindreader.png)
...
You can check out the code on [GitHub](https://github.com/abducodez/multi-agent-land), and the live demo at [Hugging Face Spaces](https://huggingface.co/spaces/build-small-hackathon/multi-agent-lab).
